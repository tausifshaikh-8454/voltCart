...
const SearchListing = () => {
    ...
    let { data: prodData, loader: prodLoading } = useFetch(productsAPI);
    let { data: blogData, loader: blogLoading } = useFetch(blogsAPI);
    let isLoader = prodLoading || blogLoading;
    let blog_filtered = (blogData || []).filter(elem => elem.blog_title.toLowerCase().includes(query.toLowerCase()))
    let prod_filtered = (prodData || []).filter(elem => elem.name.toLowerCase().includes(query.toLowerCase()))

    let results = [...prod_filtered, ...blog_filtered];
